 ...
 *
 * Version:	$Id: main.c,v 1.3 2005/12/19 12:14:12 kornet Exp $
 *
...
static const char rcsid[] = "$Id: main.c,v 1.3 2005/12/19 12:14:12 kornet Exp $";
...
static char	*name 		= "system_users_group_limit";
static char	*version 	= "0.1";
static char	*description	= "System module for IPStat Users Group Limit";
...
static NODE_GROUP_LIMIT*			find_group(IP_id id_group);
...
load(void *data)
{
    ...
    log->print_log(IP_INFO, "Starting System Module for IPStat Users Group Limits");
	...
	if (server) server->registred("users_group_limits", (void*)&server_job, NULL);
	
	if (secondary_daemon == 0) {  
		if (sched) sched->registred("sync_users_group_limit", str, (void*)&sync_limits, NULL);
   		if (sched) sched->registred("ugl_every_day", "0 0 * * *", (void*)&gl_task_every_day, api);
   		if (sched) sched->registred("ugl_every_week", "0 0 * * 1", (void*)&gl_task_every_week, api);
   		if (sched) sched->registred("ugl_every_month", "0 0 1 * *", (void*)&gl_task_every_month, api);
	}	
    return TRUE;
}
...
unload(void *data)
{
	...
	if (secondary_daemon == 0) {  
		if (sched) sched->unregistred("sync_users_group_limit");
   		if (sched) sched->unregistred("ugl_every_day");
   		if (sched) sched->unregistred("ugl_every_week");
   		if (sched) sched->unregistred("ugl_every_month");
	}	


	if (server) server->unregistred("users_group_limit");

    core->unregister_acct_hook((void*)&my_acct);
    core->unregister_autz_hook((void*)&my_autz);

    free_limits();
    log->print_log(IP_INFO, "Stoping System Module for IPStat Users Group Limits");
...
my_autz(IP_id service, void *data, void *ret)
{
	...
	if (service == SERVICE_TRAFFIC) {
		...
		LIMIT_LOCK;
			if ((node = find_group(packet_data->id_group_user)) != NULL) {
				onactive = 1;
		...
		LIMIT_LOCK;
			if ((node = find_group(squid_data->id_group_user)) != NULL) {
				onactive = 1;
				if (node->onblocked) onactive = 0; else
					if (node->limitation != -1)
				if (node->traffic >= node->limitation) onactive = 0;
...
sync_limits(void *data)
{
	...
	if ((sid = store->open())!=NULL) {
		if (!store->query(sid, "select id, md5(concat(id_group, Type, Limitation, onBlocked)) from Rules_UsersGroup")) {
			log->print_log(IP_ERR, "[users_group_limit][sync_limits]: error execute query");
			store->close(sid);
			return FALSE;
		}
		...
		mark_delete();
  		while ((row = store->fetch_row(sid))!=NULL) {
			...
			if (debug) log->print_log(IP_DEBUG, "[users_group_limit][sync_limits]: SYNC (id=%d, md5=%s)",  id, md5);
  		}  	
		if (row) free(row);
  		store->close(sid); 
		...
		log->print_log(IP_ERR, "[users_group_limit][sync_limits]: error connect to mysql");
	}			
	return TRUE;	
}
...
sync_traffic(void *data)
{
	...
	if ((sid = store->open())!=NULL) {
		...
		while ((t_tek = t_tmp) !=NULL) {
			...
			sprintf(buffer, "update Rules_UsersGroup SET Traffic=%llu, TDay=TDay+%u, TMonth=TMonth+%u, onActive='%d' where id=%d", node.traffic, (int)node.alpha, (int)node.alpha, onactive, node.id);
			if (debug) log->print_log(IP_DEBUG, "query=(%s)", buffer);
			if (!store->exec(sid, buffer)) {
				LIMIT_LOCK;
					ptr->alpha+=node.alpha;
				LIMIT_UNLOCK;
				log->print_log(IP_ERR, "[users_group_limit][sync_traffic]: error query (%s)", buffer);
			}
		}
		store->close(sid);
	}
...
traverse_print (gpointer data, gpointer user_data)
{
	...
	send_to_socket(*fd, "| %3d | %3d | %3d | %10lld | %10llu |\n\r", node->id_group, onactive, node->type, node->limitation, node->traffic);
}
...
static gint 
compare_group(gconstpointer a, gconstpointer b)
{
	NODE_GROUP_LIMIT *node1 = (NODE_GROUP_LIMIT*) a;
	NODE_GROUP_LIMIT *node2 = (NODE_GROUP_LIMIT*) b;
	
	return memcmp(&node1->id_group, &node2->id_group, sizeof(IP_id));
}
...
*find_group(IP_id id_group)
{
	...
	ptr = g_list_find_custom(st_group_limit, &node, (void*)&compare_group);
...
insert_limit(IP_id id)
{
	...
	if ((log = api->log) == NULL) return FALSE;
 	...
 	if ((sid = store->open())!=NULL) {
		...
		sprintf(buffer, "select id_group, Type, Limitation, Traffic, onBlocked, md5(concat(id_group, Type, Limitation, onBlocked)) from Rules_UsersGroup where id = %d", id);
		if (!store->query(sid, buffer)) {
			log->print_log(IP_ERR, "[users_group_limit][insert_limit]: error execute query");
			store->close(sid);
			return FALSE;
		}
		...
		if ((node = g_malloc0(sizeof(NODE_GROUP_LIMIT))) == NULL) {
			log->print_log(IP_ERR, "[users_group_limit][insert_limit]: error allocate memory for NODE_GROUP_LIMIT, skiping");
			store->close(sid); 
			return FALSE;
		}
		...
		log->print_log(IP_ERR, "[users_group_limit][insert_limit]: error connect to mysql server");	
		return FALSE;
	}
...
update_limit(IP_id id)
{
	...
	node = (NODE_GROUP_LIMIT*) ptr->data;			
 	...
 	if ((sid = store->open())!=NULL) {
		...
		sprintf(buffer, "select id_group, Type, Limitation, onBlocked, md5(concat(id_group, Type, Limitation, onBlocked)) from Rules_UsersGroup where id = %d", id);
		if (!store->query(sid, buffer)) {
			log->print_log(IP_ERR, "[users_group_limit][update_limit]: error execute query");
			store->close(sid);
			return FALSE;
		}
  		row = store->fetch_row(sid);
		...
		log->print_log(IP_ERR, "[users_group_limit][update_limit]: error connect to mysql server");	
		return FALSE;
	}
...
server_job (int fd, int argc, char **argv, void *data)
{
	if ((argc == 1) && (strncmp(argv[0], "help", 4)==0)) {
		send_to_socket(fd, "*  - show ugroup limit- Show policy for users *\n\r");
		return FALSE;
    }else if ((argc == 3) && (strncmp(argv[0], "show", 4) == 0) && (strncmp(argv[1], "ugroup", 6) == 0) && (strncmp(argv[2], "limit", 5) == 0)) {
		limits_print(fd);
		return TRUE;
    }
	return FALSE;
...
limits_print(int fd) 
{
	...
	send_to_socket(fd, "---------------------------------------------------\n\r");
    send_to_socket(fd, "| %3s | %3s | %3s | %10s | %10s |\n\r", "G", "S", "T", "Limit", "Traffic");
    send_to_socket(fd, "---------------------------------------------------\n\r");
	LIMIT_LOCK;
...
check_group_limit_tables(STORE_API *store, LOG_API *log)
{
	...
	if ((sid = store->open())!=NULL) {
		...
		if (store->list_tables(sid, "Rules_UsersGroup")) {
			if (store->fetch_row(sid) == NULL) {
				sprintf(buffer, "CREATE TABLE Rules_UsersGroup (id int(11) NOT NULL auto_increment, id_group int(11) NOT NULL default '0', Type tinyint(3) default '0', Limitation bigint(20) default '-1', Traffic bigint(20) unsigned NOT NULL default '0', YDay bigint(20) unsigned NOT NULL default '0', TDay bigint(20) unsigned NOT NULL default '0', YMonth bigint(20) unsigned NOT NULL default '0', TMonth bigint(20) unsigned NOT NULL default '0', onActive enum('0','1') NOT NULL default '1', onBlocked enum('0','1') NOT NULL default '0', PRIMARY KEY  (id), KEY key0 (id_group), KEY `key1` (`Type`,`Limitation`,`Traffic`,`onActive`,`onBlocked`)) TYPE=InnoDB");
				res = store->exec(sid, buffer);
				log->print_log(IP_INFO, "create table 'Rules_UsersGroup'");
			}
		}
...
start_label:		
		...
		if (!store->exec(sid, "start transaction")) goto error;
  		if (!store->exec(sid, "update Rules_UsersGroup SET YDay = TDay")) goto error;
		if (!store->exec(sid, "update Rules_UsersGroup SET TDay = 0")) goto error;
		if (save_timestamp(sid, store, "ugl_date_last_every_day")!=TRUE) goto error;
...
error:
		...
		log->print_log(IP_CRIT, "[users_group_limit][task_every_day]: error execute task_every_day!!!");
		return FALSE;
ok:
...
start_label:
		if (!store->exec(sid, "set autocommit=0")) goto error;
  		...
  		if (!store->exec(sid, "update Rules_UsersGroup SET YMonth = TMonth")) goto error;
		if (!store->exec(sid, "update Rules_UsersGroup SET TMonth = 0")) goto error;		
		if (save_timestamp(sid, store, "ugl_date_last_every_month")!=TRUE) goto error;
		if (!store->exec(sid, "commit")) goto error;
	  	goto ok;
error:
  		store->exec(sid, "rollback");
		...
		sleep(10);
		if (count>0) goto start_label;
		log->print_log(IP_CRIT, "[users_group_limit][task_every_month]: error execute task_every_month!!!");
		return FALSE;
ok:
		log->print_log(IP_INFO, "[users_group_limit][task_every_month]: task_every_month - success");
		store->close(sid);
    }
    return TRUE;
}
