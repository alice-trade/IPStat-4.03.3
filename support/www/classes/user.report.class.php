...
class UserReports extends Reports
{
		...
		if($this->is_connected)
		{
			...
			if(mysql_num_rows($res) == 1)
			{
				...
				<tr valign="top">
					...
					<tr bgcolor="#dddddd">
						<td colspan="2"><div class="t1"><b>Параметры</b></div></td>
					</tr>
					...
					<tr bgcolor="#ffffff" class="t1">
						<td>Лимит</td>
						<td><? if($row['Limitation'] == -1) echo "Неограничен"; else echo $this->convertTraffic($row['Limitation'])?></td>
					</tr>
					<tr bgcolor="#ffffff" class="t1">
						<td><strong>Остаток</strong></td>
						<td><strong><? if($row['Limitation'] == -1) echo "Неограничен"; else echo $this->convertTraffic($row['Limitation']-$row['Traffic'])?></strong></td>
					</tr>
					<tr bgcolor="#ffffff" class="t1">
						<td width="30%">Пароль</td>
						<td><a href="#" onclick="javascript:window.open('<?=$this->fileName?>?method=displayChangePasswd','','resizable=no,scrollbars=no,height=208,width=300,left='+(screen.width/2-150)+',top='+(screen.height/2-104))" class="t1"><strong>Сменить пароль</strong></a></td>
					</tr>	
					</table>
					<br>
					...
					<table width="100%" bgcolor="#cccccc" border="0" cellspacing="1" cellpadding="4">
					<tr bgcolor="#dddddd">
						<td colspan="2"><div class="t1"><b>Трафик</b></div></td>
					</tr>
					<tr bgcolor="#ffffff" class="t1">
						<td width="30%">Объем за период</td>
					...
					<table width="100%" bgcolor="#cccccc" border="0" cellspacing="1" cellpadding="4">
					<tr bgcolor="#dddddd">
						<td colspan="2"><div class="t1"><b>Отчет</b></div></td>
					</tr>
					<tr bgcolor="#ffffff" class="t1">
						<td width="30%">Период отчета</td>
						<td>
			...
			else $this->displayError("Невозможно определить IP-адрес");
		}
	...
	function displayChangePasswd($GET)
	{
		...
		function check() 
		{
			if(f.oldPasswd.value != '' && f.newPasswd1.value != '' && f.newPasswd2.value != '') 
			{
				...
				else alert('Не совпадает новый пароль и подтверждение');
			}
		...
		<table width="100%" bgcolor="#cccccc" border="0" cellspacing="1" cellpadding="4">
			...
			<tr>
				<td><div class="t1" align="center"><b>Изменить пароль</b></div></td>
			</tr>

			<tr bgcolor="#dddddd">
				<td width="100%"><div class="t1"><b>Старый пароль</b></div></td>
			</tr>	
			<tr bgcolor="#ffffff">
			...
			<tr bgcolor="#dddddd">
				<td width="100%"><div class="t1"><b>Новый пароль</b></div></td>
			</tr>	
			<tr bgcolor="#ffffff">
			...
			<tr bgcolor="#dddddd">
				<td width="100%"><div class="t1"><b>Подтверждение пароля</b></div></td>
			</tr>	
			<tr bgcolor="#ffffff">
			...
			<tr bgcolor="#dddddd">
				<td align="center"><input type="submit" name="btnChangePasswd" value="Изменить" onClick="check(); return false">&nbsp;&nbsp;&nbsp;<input type="button" onclick="javascript:window.close();" name="close" value="Закрыть"></td>
			</tr>
	...
	function processingChangePasswd($GET)
	{
		...
		if(mysql_num_rows($res) == 1)
		{
			...
			$errMsg = "Пароль успешно обновлен";
		}
		else $errMsg = "Вы неверно указали старый пароль";
		?>
